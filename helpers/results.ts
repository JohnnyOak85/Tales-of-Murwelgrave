...
const isLucky = (player: Player, adversary: Duelist) =>
    player.level + player.attack + player.defense <
        adversary.level + adversary.attack + adversary.defense && player.luck < LUCK_CAP;
...
const checkMonster = (player: Player, adversary: Duelist, member: GuildMember | undefined) => {
    if (
        !adversary.id.includes('_') ||
        (!isNaN(parseInt(adversary.id, 10)) && player.bestiary.includes(adversary.name))
    ) {
        return '';
    }
...
export const getBuffs = (
    ...
    reply.push(
        checkMonster(
            player,
            adversary,
            channel.guild.members.cache.find(members => members.id === player.id)
        )
    );
