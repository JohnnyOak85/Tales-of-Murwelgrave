...
const getUserRoles = (member: GuildMember) => [
    ...member.roles.cache.filter(role => !!role).values()
];
const getUserRoleNames = (member: GuildMember) =>
    getUserRoles(member).map(role => role.name.toLowerCase());
